...
`Many(scale=:Leaf, within=Subtree())`. The growth kernel obtains the live scene
with `runtime_scene(extra)`, checks its carbon and thermal thresholds, creates
...
outputs. `explain_scene_applications` should show that the new leaf is absent
