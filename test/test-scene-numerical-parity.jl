...
PlantSimEngine.meteo_inputs_(::ParityForcingModel) = (forcing=0.0,)
function PlantSimEngine.run!(::ParityForcingModel, models, status, meteo, constants, extra)
    status.var1 = meteo.forcing
    return nothing
end

PlantSimEngine.inputs_(::ParityStageOneModel) = (var1=0.0, var2=0.0)
PlantSimEngine.outputs_(::ParityStageOneModel) = (var3=0.0,)
function PlantSimEngine.run!(model::ParityStageOneModel, models, status, meteo, constants, extra)
...
PlantSimEngine.meteo_inputs_(::ParityStageTwoModel) = (T=0.0, Wind=0.0, Rh=0.0)
function PlantSimEngine.run!(::ParityStageTwoModel, models, status, meteo, constants, extra)
    status.raw_var4 = status.var3 * 2
    status.var5 = status.raw_var4 + meteo.T + 2 * meteo.Wind + 3 * meteo.Rh
    return nothing
end
...
function PlantSimEngine.run!(::ParityStageThreeModel, models, status, meteo, constants, extra)
...
function PlantSimEngine.run!(::ParityStageFiveModel, models, status, meteo, constants, extra)
...
function PlantSimEngine.run!(::ParityStageSixModel, models, status, meteo, constants, extra)
...
PlantSimEngine.run!(::ParitySoilModel, models, status, meteo, constants, extra) = nothing
...
function PlantSimEngine.run!(::ParityGatherModel, models, status, meteo, constants, extra)
...
function PlantSimEngine.run!(::ParityReceiverModel, models, status, meteo, constants, extra)
...
@testset "exact one-object process chain" begin
    ...
    run!(one_step)
    @test parity_state(only(scene_objects(one_step)).status) == [34.95, 22.0, 56.95, 15.0, 5.5, 0.3]
    ...
    @test parity_state(only(scene_objects(two_steps)).status) == [40.0, 23.2, 63.2, 16.0, 5.8, 0.3]
    @test last.(scene_outputs(simulation)[(:stage_three, ObjectId(:leaf), :var6)]) == [56.95, 63.2]
end
...
@testset "one application with an object-specific Override" begin
    ...
    scene = Scene(instance; environment=parity_weather)
    simulation = run!(scene; steps=2, outputs=:all)
...
@testset "exact multiscale gather and scatter chain" begin
    ...
    simulation = run!(scene; steps=2, outputs=:all)
    leaves = scene_objects(scene; scale=:Leaf)
