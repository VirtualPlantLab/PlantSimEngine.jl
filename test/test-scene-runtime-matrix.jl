...
@testset "environment and output request matrix" begin
    ...
    @test last.(scene_outputs(constant_sim)[(:probe, ObjectId(:probe), :seen)]) == [12.0, 12.0]

    one_row_scene = runtime_matrix_scene([(T=13.0, duration=Hour(1))])
    one_row_sim = run!(one_row_scene)
    @test only(scene_objects(one_row_scene)).status.seen == 13.0
    ...
    canonical = scene_outputs(selected_sim)[(:probe, ObjectId(:probe), :seen)]
    ...
    @test isempty(scene_outputs(no_outputs))
    ...
    @test last.(scene_outputs(continued_sim)[
