...
`ModelSpec` is the user-facing API for mapping-level coupling configuration.
Bindings are read from `ModelSpec` during multiscale multi-rate simulation.

Expected return value is a `NamedTuple` keyed by consumer input variable names.
Each value must itself be a `NamedTuple` with:
...
This lets a consumer input read from a producer variable even when names differ.
For example, consumer input `:C` can be mapped from producer output `:S`.
