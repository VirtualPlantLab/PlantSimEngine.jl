...
"""
    compile_scene(scene[, applications...])

Compile scene applications, selectors, value bindings, hard calls, writer
ordering, and schedules into the single Scene/Object runtime representation.
...
function _compile_scene(scene::Scene, raw_specs; validate_required_inputs::Bool=true)
...
function _validate_scene_call_cadences!(applications, call_bindings, timeline)
    applications_by_id = Dict(application.id => application for application in applications)
...
"""
    explain_initialization(scene::Scene)
...
Unlike [`compile_scene`](@ref), this report does not fail solely because a
...
"""
function explain_initialization(scene::Scene)
    supplied = Dict(
    ...
    environment_bindings = Dict(
        ...
        for binding in _compile_environment_bindings(scene, compiled)
    )
...
function _compile_scene_applications(scene::Scene, raw_specs, timeline)
    ...
    for spec in specs
        ...
        if isnothing(name) && process_counts[proc] > 1
            error(
                "Scene contains $(process_counts[proc]) unnamed applications for process `$(proc)`. ",
...
function _update_matches_application(label::Symbol, application::CompiledSceneApplication)
        ...
        for (_, application) in indexed_writers
            ...
            if !isempty(previous) &&
               !_updates_after_previous_writer(
                   application.spec,
                   variable,
                   CompiledSceneApplication[last(previous)],
               )
...
function _scene_selector_policy(selector::AbstractObjectMultiplicity, applications_by_id, source_application_ids, source_var::Symbol)
    if _selector_has_policy(selector)
        ...
        return _as_schedule_policy(
            policy;
            context="scene input policy for `$(source_var)`",
        )
    end
    ...
    return [
        (
            ...
            target_scales=sort!(unique!(Symbol[
                _scene_object(compiled.scene, id).scale
                for id in application.target_ids
                if !isnothing(_scene_object(compiled.scene, id).scale)
            ]); by=string),
...
explain_scene_applications(scene::Scene) =
    explain_scene_applications(refresh_bindings!(scene))
...
explain_schedule(scene::Scene) = explain_schedule(refresh_bindings!(scene))
...
explain_bindings(scene::Scene) = explain_bindings(refresh_bindings!(scene))
...
explain_calls(scene::Scene) = explain_calls(refresh_bindings!(scene))
...
explain_model_bundles(scene::Scene) =
    explain_model_bundles(refresh_bindings!(scene))
...
explain_writers(scene::Scene) = explain_writers(refresh_bindings!(scene))
